	...
	setup: function(proto) {
		...
		if (!clientid) {
			...
			if (!clientid) {
				...
				clientid = cursor.get('network', '@globals[0]', 'dhcp_default_duid');
			}
		}
		if (clientid)
			push(argv, `-c${clientid}`);

		if (cfg.defaultreqopts == '0')
			push(argv, '-R');

		if (cfg.noslaaconly == '1')      push(argv, '-S');
		if (cfg.forceprefix == '1')      push(argv, '-F');
		if (cfg.norelease == '1')        push(argv, '-k');
		if (cfg.strict_rfc7550 == '1')   push(argv, '--strict-rfc7550');
		if (cfg.noserverunicast == '1')   push(argv, '-U');
		if (cfg.noclientfqdn == '1')     push(argv, '-f');
		if (cfg.noacceptreconfig == '1')  push(argv, '-a');
		if (cfg.keep_ra_dnslifetime == '1') push(argv, '-L');
		...
		if (cfg.sourcefilter == '0')
			push(env, 'NOSOURCEFILTER=1');
		if (cfg.extendprefix == '1')
